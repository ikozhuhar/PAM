...
![image](https://github.com/user-attachments/assets/a80bc4f0-0b6c-4940-9ba0-089e5c605dfb)
...
Все конфигурационные файлы PAM для различных приложений находятся в папке /etc/pam.d.

```
cat /etc/pam.d/
```
![image](https://github.com/user-attachments/assets/8cfeb91f-e28a-4f16-ae5a-6c4099aaadf6)

Cмотрим конфигурационный файл для утилиты sudo:
```
cat /etc/pam.d/sudo
```

![image](https://github.com/user-attachments/assets/6344ea84-fb8a-4904-a4ee-5b164ed45765)
...
Кроме того, существуют директивы include, которые включают строки из других файлов так, как будто они были написаны в этом файле. Файл настройки sudo кажется совсем коротким, но в него включаются другие файлы с помощью include. 
...
## [[⬆]](#toc) <a name='7'>Модули</a>

```ruby
pam_rootok.so
Модуль проверяет, что пользователь – root (uid 0), и в таком случае сразу разрешает доступ без дальнейшей аутентификации.
​
pam_unix.so
Реализует стандартную аутентификацию по Unix-паролю, а также ведёт учёт и обновление паролей через /etc/passwd и /etc/shadow.
​
pam_nologin.so
Запрещает вход обычным пользователям, если существует файл /etc/nologin; сообщение из этого файла показывается при отказе.
​
pam_gdm.so
Специализированный модуль для дисплей-менеджера GDM: участвует в аутентификации и сессии графического входа GNOME Display Manager.

pam_gnome_keyring.so
Автоматически разблокирует и запускает GNOME Keyring при входе пользователя, чтобы приложения могли использовать хранилище ключей без дополнительного ввода пароля.
​
pam_loginuid.so
Устанавливает loginuid процесса (идентификатор «владельца» сессии) для подсистемы аудита Linux, чтобы все события аудита привязывались к вошедшему пользователю.
​
pam_limits.so
Применяет ресурсные лимиты (ulimit: число процессов, размер памяти, файлов и т.п.) из /etc/security/limits.conf к пользовательской сессии.
​
pam_pkcs11.so
Позволяет использовать PKCS#11‑совместимые смарт‑карты/токены для аутентификации вместо пароля или вместе с ним.
​
pam_permit.so
Безусловно разрешает прохождение данного шага PAM; обычно используется как «заглушка» или в сочетании с другими правилами.

pam_deny.so
Безусловно запрещает доступ на данном шаге; применяется для жёсткого закрытия пути при попадании в определённую ветку стека PAM.
​
pam_umask.so
Устанавливает umask (маску прав для новых файлов и каталогов) при старте сессии в соответствии с настройками, часто из /etc/login.defs или pam.d.
​
pam_group.so
Автоматически добавляет пользователя во временные дополнительные группы на время сессии согласно /etc/security/group.conf (например, для доступа к устройствам).
​
pam_time.so
Ограничивает доступ пользователей и групп по времени и дню недели, используя правила из /etc/security/time.conf.
​
pam_access.so
Ограничивает вход по правилам в /etc/security/access.conf (по имени пользователя, группе, хосту/tty и т.п.).
​
pam_lastlog.so
Показывает информацию о последнем входе (дата, tty, хост) и/или записывает текущий вход в файлы учёта (обычно /var/log/lastlog и т.п.).
​
pam_shells.so
Проверяет, что логин‑шелл пользователя из /etc/passwd присутствует в /etc/shells; если нет — может запретить вход.
​
pam_systemd.so
Интеграция с systemd: создаёт и привязывает сессию пользователя, логин юнит, может настраивать окружение, логирование и cgroup для сессии.
​
pam_env.so
Устанавливает и модифицирует переменные окружения при аутентификации и начале сессии на основе /etc/security/pam_env.conf и других файлов.
​
pam_fprintd.so
Позволяет использовать отпечаток пальца (через демон fprintd) для аутентификации как альтернативу или дополнение к паролю.
​
pam_wheel.so
Ограничивает возможность использования su (или других сервисов) только членами группы wheel (или другой заданной группы).
```
